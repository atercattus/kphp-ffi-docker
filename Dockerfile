...
RUN git clone --depth 1 --branch v1.10.x https://github.com/google/googletest.git \
    && cd googletest \
    && cmake . \
    && make -j$(nproc) \
    && make install

RUN git clone https://github.com/VKCOM/kphp.git \
	&& cd kphp \
	&& git fetch -a \
...
WORKDIR /opt/kphp
ENV PATH "/opt/kphp/objs/bin:${PATH}"
